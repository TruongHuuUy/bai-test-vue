...
const addTaskInDate = () => {
    listTasksFiltered.value = getDataFromLocalStorage()
    for (let item of dateOfMonth.value) {
        for (let itemTask of listTasksFiltered.value) {
            item.task = filterEndDateEqualFullDate(listTasksFiltered.value, item.fullDate);
            if (item.fullDate === itemTask.endDay) {
                if (itemTask.status === true) {
                    item.checkTaskDone = true

                    item.countTaskDone = filterDoneTask(item.task).length
                }
                else if (itemTask.endDay < getDateLocal) {
                    item.checkTaskLimit = true
                    item.countTaskLimit = filterTimeLimitTask(item.task).length
                }
                else if (itemTask.status === false) {
                    item.checkTaskDoNot = true
                    item.countTaskDoNot = filterDoNotTask(item.task).length
                }
            }
        }
    }
